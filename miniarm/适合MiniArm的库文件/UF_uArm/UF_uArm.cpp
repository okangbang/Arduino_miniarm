...
//    pinMode(LIMIT_SW, INPUT);  digitalWrite(LIMIT_SW, HIGH);  //限位开关
//    pinMode(BTN_D4,   INPUT);  digitalWrite(BTN_D4,   HIGH);  //按钮d4
//    pinMode(BTN_D7,   INPUT);  digitalWrite(BTN_D7,   HIGH);  //按钮d7
//    pinMode(BUZZER,   OUTPUT); digitalWrite(BUZZER,   LOW);  //蜂鸣器
//    pinMode(PUMP_EN,  OUTPUT); digitalWrite(PUMP_EN,  LOW);  //放气阀 （快速解除管路负压状态）//暂无
//   pinMode(VALVE_EN, OUTPUT); digitalWrite(VALVE_EN, LOW);  //真空泵 （用吸盘吸取物件）      //暂无

//	if (EEPROM.read(0) == CALIBRATION_FLAG) // read of offset flag  从eprom读修正值   //因未设置所以忽律此判断，用默认值初始化舵机
    {
		// attaches the servo on pin to the servo object
		servoL.attach(SERVO_L, D150A_SERVO_MIN_PUL, D150A_SERVO_MAX_PUL); //左侧大臂舵机
		servoR.attach(SERVO_R, D150A_SERVO_MIN_PUL, D150A_SERVO_MAX_PUL); //右侧小臂舵机
		servoRot.attach(SERVO_ROT, D150A_SERVO_MIN_PUL, D150A_SERVO_MAX_PUL); // 底座旋转舵机
		servoHand.attach(SERVO_HAND, D009A_SERVO_MIN_PUL, D009A_SERVO_MAX_PUL); // 爪舵机
		//servoHandRot.attach(SERVO_HAND_ROT, D009A_SERVO_MIN_PUL, D009A_SERVO_MAX_PUL); // 旋转爪舵机 //暂无
		servoHand.write(HAND_ANGLE_OPEN, 0, true);  //打开爪
		servoHand.detach(); //断开爪舵机，使之放松

		//servoL.write(map(readAngle(SERVO_L), SERVO_MIN, SERVO_MAX, 0, 180));  //原语句读摆臂位置，改为默认90度位置 下同
		...
		servoL.write(map(90, SERVO_MIN, SERVO_MAX, 0, 180));  //原语句如上
		servoR.write(map(90, SERVO_MIN, SERVO_MAX, 0, 180));
		servoRot.write(map(90, SERVO_MIN, SERVO_MAX, 0, 180));
		...
		setPosition(stretch, height, rotation, handRot); //设置摆臂位置 init前数值都是0
    }
/*    else
    {	//读EEPROM失败 不初始化舵机，报警 buzzer alert if calibration needed 
		alert(3, 200, 200);
    }*/
}

/*void UF_uArm::calibration() //校准；刻度；标度
{
...
{   
	...
	//_handRot = constrain(_handRot, HAND_ROTATION_MIN, HAND_ROTATION_MAX) + 90;	// +90, change -90~90 to 0~180 //爪旋转（暂无）
	// angle calculation
	...
	// angle limit
	angleL = constrain(angleL, 10 + offsetL, 145 + offsetL); //求极限，让angleL 最小等于10+offset ，最大不超过 145+offset
	...
	servoRot.write( 180 - _armRot, servoSpdRot, false); //使用齿轮后，需要将角度翻转，所以增加  180- 
